...
## 2025-10-24 - Pandas Series apply(func) overhead
**Learning:** Using `Series.apply(func)` on a Pandas Series for applying custom Python functions (like string manipulations) is significantly slower than using a simple list comprehension `[func(x) for x in Series]`. `apply` incurs massive overhead checking types and repeatedly crossing the C/Python boundary, whereas a list comprehension runs directly in native Python.
**Action:** When applying custom string manipulation functions or simple non-numpy functions over a Pandas Series, avoid `.apply()`. Extract the series values and use a list comprehension instead.
